...
TABLE_NAME_MONTHLY = "bhavya_month_wise_health_report"
TABLE_NAME_YEARLY = "bhavya_year_wise_health_report"
...
def get_create_table_monthly_sql():
    """Generate CREATE TABLE SQL for monthly table (without data_date)"""
    return f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME_MONTHLY} (
        id INT AUTO_INCREMENT PRIMARY KEY,
        state_name TEXT, focus_area TEXT, year TEXT, month TEXT,
        number_of_abdm_cards_linked TEXT, number_of_abdm_cards_shared TEXT,
        number_of_abdm_cards_created TEXT, number_of_abdm_health_facility_registry TEXT,
        ...
        source TEXT, citizen_portal_live_facility_count TEXT,
        number_of_wards TEXT, number_of_beds TEXT, fetched_at TEXT,
        UNIQUE KEY unique_year_month_idx (year(10), month(20))
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
    """

def get_create_table_yearly_sql():
    """Generate CREATE TABLE SQL for yearly table (without data_date and month)"""
    return f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME_YEARLY} (
        id INT AUTO_INCREMENT PRIMARY KEY,
        state_name TEXT, focus_area TEXT, year TEXT,
        number_of_abdm_cards_linked TEXT, number_of_abdm_cards_shared TEXT,
        number_of_abdm_cards_created TEXT, number_of_abdm_health_facility_registry TEXT,
        abdm_healthcare_professionals_registry TEXT, number_of_doctors TEXT,
        number_of_nurses TEXT, number_of_data_entry_operators TEXT,
        ...
        source TEXT, citizen_portal_live_facility_count TEXT,
        number_of_wards TEXT, number_of_beds TEXT, fetched_at TEXT,
        UNIQUE KEY unique_year_idx (year(10))
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
    """

# Columns for monthly table (without data_date)
TARGET_COLUMNS_MONTHLY = [
    "state_name", "focus_area", "year", "month",
    "number_of_abdm_cards_linked", "number_of_abdm_cards_shared", "number_of_abdm_cards_created",
    "number_of_abdm_health_facility_registry", "abdm_healthcare_professionals_registry",
    "number_of_doctors", "number_of_nurses", "number_of_data_entry_operators",
    "number_of_pharmacists", "number_of_lab_attendents", "number_of_community_health_officers",
...
# Columns for yearly table (without data_date and month)
TARGET_COLUMNS_YEARLY = [
    "state_name", "focus_area", "year",
    "number_of_abdm_cards_linked", "number_of_abdm_cards_shared", "number_of_abdm_cards_created",
    "number_of_abdm_health_facility_registry", "abdm_healthcare_professionals_registry",
    "number_of_doctors", "number_of_nurses", "number_of_data_entry_operators",
    "number_of_pharmacists", "number_of_lab_attendents", "number_of_community_health_officers",
...
class MonthRangeRequest(BaseModel):
    from_year: int
    from_month: int
    to_year: int
    to_month: int
...
class DataFetcher:
    ...
    def process_data_monthly(self, all_data, year, month, start_date, end_date):
        """Process data for monthly table (without data_date)"""
        row = {col: "Not Available" for col in TARGET_COLUMNS_MONTHLY}
        ...
        # Same data extraction logic as daily
        if all_data.get("staff_data"):
            d = all_data["staff_data"]
            row["number_of_doctors"] = self.safe_get(d, "doctor")
            row["number_of_nurses"] = self.safe_get(d, "nurse")
    ...
    def process_data_yearly(self, all_data, year, start_date, end_date):
        """Process data for yearly table (without data_date and month)"""
        row = {col: "Not Available" for col in TARGET_COLUMNS_YEARLY}
...
class DatabaseManager:
    ...
    @staticmethod
    def table_exists_monthly(conn):
        """Check if monthly table exists"""
        try:
            cursor = conn.cursor()
            cursor.execute(f"SHOW TABLES LIKE '{TABLE_NAME_MONTHLY}'")
            result = cursor.fetchone()
            cursor.close()
            exists = result is not None
            print(f"Table '{TABLE_NAME_MONTHLY}' exists: {exists}")
            return exists
        except Exception as e:
            print(f"Monthly table check error: {e}")
            return False

    @staticmethod
    def create_table_monthly(conn):
        """Create the monthly table if it doesn't exist"""
        try:
            cursor = conn.cursor()
            print(f"Creating table {TABLE_NAME_MONTHLY}...")
            create_sql = get_create_table_monthly_sql()
            cursor.execute(create_sql)
            conn.commit()
            cursor.close()
            if DatabaseManager.table_exists_monthly(conn):
                print(f"Table {TABLE_NAME_MONTHLY} created successfully!")
                return True
            return False
        except Error as e:
            print(f"Monthly table creation error: {e}")
            try:
                conn.rollback()
            except:
                pass
    ...
    @staticmethod
    def ensure_table_exists_monthly(conn):
        """Check if monthly table exists, create if not"""
        if DatabaseManager.table_exists_monthly(conn):
            return True
        return DatabaseManager.create_table_monthly(conn)

    @staticmethod
    def check_duplicate_monthly(conn, year, month):
        """Check if year-month combination already exists"""
        try:
            cursor = conn.cursor()
            cursor.execute(f"SELECT COUNT(*) FROM {TABLE_NAME_MONTHLY} WHERE year = %s AND month = %s", (str(year), month))
            count = cursor.fetchone()[0]
            cursor.close()
            return count > 0
        except Error as e:
            print(f"Monthly duplicate check error: {e}")
            return False

    @staticmethod
    def insert_data_monthly(conn, row_data):
        """Insert data into monthly table"""
        try:
            year = row_data.get("year")
            month = row_data.get("month")
            if DatabaseManager.check_duplicate_monthly(conn, year, month):
                print(f"Duplicate found for {year}-{month}, skipping")
                return False

            columns = ", ".join(row_data.keys())
            placeholders = ", ".join(["%s"] * len(row_data))
            query = f"INSERT IGNORE INTO {TABLE_NAME_MONTHLY} ({columns}) VALUES ({placeholders})"
            cursor = conn.cursor()
            cursor.execute(query, list(row_data.values()))
            conn.commit()
            affected = cursor.rowcount
            cursor.close()
            print(f"Monthly insert result: {affected} row(s) affected")
            return affected > 0
        except Error as e:
            print(f"Monthly insert error: {e}")
            try:
                conn.rollback()
            except:
                pass
    ...
    @staticmethod
    def get_record_count_monthly(conn):
        try:
            cursor = conn.cursor()
            cursor.execute(f"SELECT COUNT(*) FROM {TABLE_NAME_MONTHLY}")
            result = cursor.fetchone()
            cursor.close()
            return result[0] if result else 0
        except Exception as e:
            print(f"Monthly record count error: {e}")
            return 0

    # ============== YEARLY TABLE METHODS ==============
    @staticmethod
    def table_exists_yearly(conn):
        """Check if yearly table exists"""
        try:
            cursor = conn.cursor()
            cursor.execute(f"SHOW TABLES LIKE '{TABLE_NAME_YEARLY}'")
            result = cursor.fetchone()
            cursor.close()
            exists = result is not None
            print(f"Table '{TABLE_NAME_YEARLY}' exists: {exists}")
            return exists
        except Exception as e:
            print(f"Yearly table check error: {e}")
            return False

    @staticmethod
    def create_table_yearly(conn):
        """Create the yearly table if it doesn't exist"""
        try:
            cursor = conn.cursor()
            print(f"Creating table {TABLE_NAME_YEARLY}...")
            create_sql = get_create_table_yearly_sql()
            cursor.execute(create_sql)
            conn.commit()
            cursor.close()
            if DatabaseManager.table_exists_yearly(conn):
                print(f"Table {TABLE_NAME_YEARLY} created successfully!")
                return True
            return False
        except Error as e:
            print(f"Yearly table creation error: {e}")
            try:
                conn.rollback()
            except:
                pass
    ...
    @staticmethod
    def ensure_table_exists_yearly(conn):
        """Check if yearly table exists, create if not"""
        if DatabaseManager.table_exists_yearly(conn):
            return True
        return DatabaseManager.create_table_yearly(conn)

    @staticmethod
    def check_duplicate_yearly(conn, year):
        """Check if year already exists"""
        try:
            cursor = conn.cursor()
            cursor.execute(f"SELECT COUNT(*) FROM {TABLE_NAME_YEARLY} WHERE year = %s", (str(year),))
            count = cursor.fetchone()[0]
            cursor.close()
            return count > 0
        except Error as e:
            print(f"Yearly duplicate check error: {e}")
            return False

    @staticmethod
    def insert_data_yearly(conn, row_data):
        """Insert data into yearly table"""
        try:
            year = row_data.get("year")
            if DatabaseManager.check_duplicate_yearly(conn, year):
                print(f"Duplicate found for year {year}, skipping")
                return False

            columns = ", ".join(row_data.keys())
            placeholders = ", ".join(["%s"] * len(row_data))
            query = f"INSERT IGNORE INTO {TABLE_NAME_YEARLY} ({columns}) VALUES ({placeholders})"
            cursor = conn.cursor()
            cursor.execute(query, list(row_data.values()))
            conn.commit()
            affected = cursor.rowcount
            cursor.close()
            print(f"Yearly insert result: {affected} row(s) affected")
            return affected > 0
        except Error as e:
            print(f"Yearly insert error: {e}")
            try:
                conn.rollback()
            except:
                pass
    ...
    @staticmethod
    def get_record_count_yearly(conn):
        try:
            cursor = conn.cursor()
            cursor.execute(f"SELECT COUNT(*) FROM {TABLE_NAME_YEARLY}")
            result = cursor.fetchone()
            cursor.close()
            return result[0] if result else 0
        except Exception as e:
            print(f"Yearly record count error: {e}")
            return 0
...
@app.post("/api/fetch/monthly")
async def fetch_monthly(request: MonthRangeRequest):
    """Fetch data for a range of months"""
    try:
        # Validate month range
        if request.from_month < 1 or request.from_month > 12 or request.to_month < 1 or request.to_month > 12:
            raise HTTPException(400, "Month must be between 1 and 12")

        # Check if from is before or equal to to
        from_tuple = (request.from_year, request.from_month)
        to_tuple = (request.to_year, request.to_month)
        if from_tuple > to_tuple:
            raise HTTPException(400, "From month cannot be after To month")
        ...
        while (current_year, current_month) <= to_tuple:
        ...
        return StreamingResponse(
            fetch_data_stream_monthly(months),
            media_type="text/event-stream"
        )
...
@app.post("/api/fetch/yearly")
async def fetch_yearly(request: YearRangeRequest):
    """Fetch data for a range of years"""
    try:
        if request.from_year > request.to_year:
            raise HTTPException(400, "From year cannot be after To year")

        # Generate list of years
        years = list(range(request.from_year, request.to_year + 1))

        return StreamingResponse(
            fetch_data_stream_yearly(years),
            media_type="text/event-stream"
        )
    except ValueError as e:
        raise HTTPException(400, f"Invalid input: {str(e)}")
...
async def fetch_data_stream_monthly(months):
    """Generator for SSE streaming for monthly data"""
    import calendar
    ...
    yield f"data: {json.dumps({'type': 'log', 'message': 'Checking monthly database table...'})}\n\n"
    await asyncio.sleep(0.1)

    if not DatabaseManager.ensure_table_exists_monthly(conn):
        yield f"data: {json.dumps({'type': 'error', 'message': 'Failed to create monthly database table'})}\n\n"
        conn.close()
        return

    yield f"data: {json.dumps({'type': 'log', 'message': 'Monthly database table ready'})}\n\n"
    await asyncio.sleep(0.1)

    # Send start event
    yield f"data: {json.dumps({'type': 'start', 'total_dates': len(months), 'total_endpoints': len(ENDPOINTS), 'fetch_type': 'monthly'})}\n\n"
    await asyncio.sleep(0.1)
    ...
    executor = ThreadPoolExecutor(max_workers=34)
    ...
    for month_idx, (year, month) in enumerate(months):
        ...
        yield f"data: {json.dumps({'type': 'date_start', 'date': display_name, 'date_index': month_idx + 1, 'total_dates': len(months)})}\n\n"
        await asyncio.sleep(0.1)

        # Check duplicate before fetching
        if DatabaseManager.check_duplicate_monthly(conn, year, month_name):
            yield f"data: {json.dumps({'type': 'date_skip', 'date': display_name, 'reason': 'Already exists in database'})}\n\n"
            skip_count += 1
            continue

        yield f"data: {json.dumps({'type': 'log', 'message': f'Fetching all 34 endpoints concurrently for {display_name}...'})}\n\n"
        await asyncio.sleep(0.1)
        ...
        # Process and save
        yield f"data: {json.dumps({'type': 'log', 'message': f'Processing data for {display_name}...'})}\n\n"
        row_data = fetcher.process_data_monthly(all_data, year, month_name, start_date, end_date)

        if DatabaseManager.insert_data_monthly(conn, row_data):
            yield f"data: {json.dumps({'type': 'date_done', 'date': display_name, 'status': 'success', 'message': 'Data saved to database'})}\n\n"
            success_count += 1
        else:
            yield f"data: {json.dumps({'type': 'date_done', 'date': display_name, 'status': 'failed', 'message': 'Failed to save to database'})}\n\n"
            fail_count += 1
    ...
    total_records = DatabaseManager.get_record_count_monthly(conn)
    conn.close()

    yield f"data: {json.dumps({'type': 'complete', 'success': success_count, 'skipped': skip_count, 'failed': fail_count, 'total_records': total_records, 'fetch_type': 'monthly'})}\n\n"


async def fetch_data_stream_yearly(years):
    """Generator for SSE streaming for yearly data"""
    fetcher = DataFetcher()
    conn = DatabaseManager.get_connection()
    ...
    # Ensure yearly table exists
    yield f"data: {json.dumps({'type': 'log', 'message': 'Checking yearly database table...'})}\n\n"
    await asyncio.sleep(0.1)

    if not DatabaseManager.ensure_table_exists_yearly(conn):
        yield f"data: {json.dumps({'type': 'error', 'message': 'Failed to create yearly database table'})}\n\n"
        conn.close()
        return

    yield f"data: {json.dumps({'type': 'log', 'message': 'Yearly database table ready'})}\n\n"
    await asyncio.sleep(0.1)

    # Send start event
    yield f"data: {json.dumps({'type': 'start', 'total_dates': len(years), 'total_endpoints': len(ENDPOINTS), 'fetch_type': 'yearly'})}\n\n"
    await asyncio.sleep(0.1)
    ...
    for year_idx, year in enumerate(years):
        ...
        yield f"data: {json.dumps({'type': 'date_start', 'date': display_name, 'date_index': year_idx + 1, 'total_dates': len(years)})}\n\n"
        await asyncio.sleep(0.1)
        ...
        # Process and save
        yield f"data: {json.dumps({'type': 'log', 'message': f'Processing data for {display_name}...'})}\n\n"
        row_data = fetcher.process_data_yearly(all_data, year, start_date, end_date)

        if DatabaseManager.insert_data_yearly(conn, row_data):
            yield f"data: {json.dumps({'type': 'date_done', 'date': display_name, 'status': 'success', 'message': 'Data saved to database'})}\n\n"
            success_count += 1
        else:
            yield f"data: {json.dumps({'type': 'date_done', 'date': display_name, 'status': 'failed', 'message': 'Failed to save to database'})}\n\n"
    ...
    total_records = DatabaseManager.get_record_count_yearly(conn)
    conn.close()

    yield f"data: {json.dumps({'type': 'complete', 'success': success_count, 'skipped': skip_count, 'failed': fail_count, 'total_records': total_records, 'fetch_type': 'yearly'})}\n\n"
